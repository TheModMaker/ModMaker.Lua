...
namespace UnitTests.Parser
{
    ...
    {
        static Lexer CreateLexer(string str)
        {
...
not an escape \n]==]
        ...
        public void StringErrorTest()
        {
            ...
            Assert.Throws<SyntaxException>(
                () => CreateLexer("foo`").Read());
            Assert.Throws<SyntaxException>(
                () => CreateLexer("foo`e").Read());
        }
        ...
        {
            Token tok = CreateLexer(@"'\'\""\\\n\a\b\f\r\t\v\x12\73\z    '").Read();
            Assert.AreEqual(tok.Value, "'\"\\\n\a\b\f\r\t\v\x12\x3b");

            Assert.Throws<SyntaxException>(() => CreateLexer("'\\w'").Read());
            Assert.Throws<SyntaxException>(() => CreateLexer("'\\q'").Read());
        }

        [Test]
        public void CommentErrorTest()
        {
            Assert.Throws<SyntaxException>(() => CreateLexer("--[[ ").Read());
            Assert.Throws<SyntaxException>(() => CreateLexer("--[[ ]").Read());
            Assert.Throws<SyntaxException>(() => CreateLexer("--[[ ]=]").Read());
        }
    }
