...
namespace UnitTests.Runtime.LuaLibraries {
  ...
  public class String : LibraryTestBase {
    ...
    public void byte_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.String, "string.byte({0})");
      _runInvalidTypeTests(LuaValueType.Number, "string.byte('cat', {0})");
      _runInvalidTypeTests(LuaValueType.Number, "string.byte('cat', 3, {0})", allowNil: true);
    }
    #endregion
    ...
    public void char_() {
      _lua.DoText(@"
        ...
        assertEquals('" + "\ud801\udc37" + @"',   string.char(0xd801, 0xdc37),
                     'char: handles surrogate pairs split')
        assertEquals('" + "\ud801\udc37" + @"',   string.char(0x10437),
                     'char: handles high code points')
      ");
    }

    [Test]
    public void char_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.Number, "string.char({0})");
      _runInvalidTypeTests(LuaValueType.Number, "string.char(3, 4, {0}, 1)");
    }

    [Test]
    public void char_BadArgument() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.char(2, -1)");
      });
    }
    #endregion
    ...
    public void find() {
      _lua.DoText(@"
        ...
        assertEquals(0,     select('#', string.find('aXabcd', 'X', 3)),
                     'find: not found, with start')
    ...
    public void find_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.String, "string.find({0}, 'cat')");
      _runInvalidTypeTests(LuaValueType.String, "string.find('cat', {0})");
      _runInvalidTypeTests(LuaValueType.Number, "string.find('cat', 'cat', {0})");
    }

    [Test]
    public void find_NotEnoughArgs() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.find()");
      });
    }
    ...
    public void find_NotEnoughArgs2() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.find('foobar')");
      });
    }
    #endregion
    ...
    public void format_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.String, "string.format({0})");
    }

    [Test]
    public void format_NoArgs() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.format()");
      });
    }
    #endregion
    ...
    public void gmatch_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.String, "string.gmatch({0}, 'cat')");
      _runInvalidTypeTests(LuaValueType.String, "string.gmatch('cat', {0})");
    }

    [Test]
    public void gmatch_NotEnoughArgs() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.gmatch()");
      });
    }
    ...
    public void gsub() {
      _lua.DoText(@"
        ...
        assertEquals('abc abc',        string.gsub('123 abc', '(\\w+)', repl, 1),
                     'gsub: function')
        assertEquals('makes lua code', string.gsub('makes #xyz #abc', '#(\\w+)', t),
                     'gsub: table')
      ");
    }
    ...
    public void gsub_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.String, "string.gsub({0}, 'cat', '')");
      _runInvalidTypeTests(LuaValueType.String, "string.gsub('cat', {0}, '')");
    }

    [Test]
    public void gsub_NotEnoughArgs() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.gsub()");
      });
    }
    ...
    public void gsub_NotEnoughArgs3() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.gsub('cat', 'cat')");
      });
    }
    #endregion
    ...
    public void len_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.String, "string.len({0})");
    }

    [Test]
    public void len_NotEnoughArgs() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.len()");
      });
    }
    #endregion
    ...
    public void lower_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.String, "string.lower({0})");
    }

    [Test]
    public void lower_NotEnoughArgs() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.lower()");
      });
    }
    #endregion
    ...
    public void match_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.String, "string.match({0}, 'cat')");
      _runInvalidTypeTests(LuaValueType.String, "string.match('cat', {0})");
      _runInvalidTypeTests(LuaValueType.Number, "string.match('cat', 'c', {0})", allowNil: true);
    }

    [Test]
    public void match_NotEnoughArgs() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.match()");
      });
    }
    ...
    public void rep_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.String, "string.rep({0}, 8)");
      _runInvalidTypeTests(LuaValueType.Number, "string.rep('cat', {0})");
      _runInvalidTypeTests(LuaValueType.String, "string.rep('cat', 8, {0})", allowNil: true);
    }

    [Test]
    public void rep_NotEnoughArgs() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.rep()");
      });
    }
    ...
    public void reverse() {
      _lua.DoText(@"
        ...
        assertEquals('" + "\u2678a\u4623" + @"', string.reverse('" + "\u4623a\u2678" + @"'),
                     'reverse: supports Unicode')
        assertEquals('" + "\ud801\udc37a" + @"', string.reverse('" + "a\ud801\udc37" + @"'),
                     'reverse: supports UTF-16')
      ");
    }

    [Test]
    public void reverse_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.String, "string.reverse({0})");
    }

    [Test]
    public void reverse_NotEnoughArgs() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.reverse()");
      });
    }
    #endregion
    ...
    public void sub_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.String, "string.sub({0}, 3)");
      _runInvalidTypeTests(LuaValueType.Number, "string.sub('cat', {0})");
      _runInvalidTypeTests(LuaValueType.Number, "string.sub('cat', 2, {0})", allowNil: true);
    }

    [Test]
    public void sub_NotEnoughArgs() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.sub()");
      });
    }
    ...
    public void upper_InvalidTypes() {
      _runInvalidTypeTests(LuaValueType.String, "string.upper({0})");
    }

    [Test]
    public void upper_NotEnoughArgs() {
      Assert.Throws<ArgumentException>(() => {
        _lua.DoText(@"string.upper()");
      });
    }
    #endregion
  }
